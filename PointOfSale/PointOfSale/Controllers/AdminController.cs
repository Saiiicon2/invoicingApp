...
namespace PointOfSale.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        {
            // Fix 1: Consistent ViewBag usage (capitalize correctly)
            ViewBag.TotalSales = await _dashboardService.TotalSalesLastWeek();
            ViewBag.TotalRevenue = await _dashboardService.TotalIncomeLastWeek(); // Fix 4: Corrected TotalIncome to TotalRevenue
            ViewBag.TotalProducts = await _dashboardService.TotalProducts();
            ViewBag.TotalOrders = await _dashboardService.TotalCategories(); // Fix 2: Set TotalOrders properly

            // Fix 3: Pass metrics to the view using ViewBag or ViewData
            var metrics = new List<DashboardMetric>
                {
                    new DashboardMetric { Title = "Total Sales", Value = ViewBag.TotalSales.ToString(), Icon = "shopping-basket", Color = "primary" },
                    new DashboardMetric { Title = "Total Revenue", Value = $"R{ViewBag.TotalRevenue:N0}", Icon = "money-bill-wave", Color = "success" },
                    new DashboardMetric { Title = "Total Products", Value = ViewBag.TotalProducts.ToString(), Icon = "clipboard-list", Color = "info" },
                    new DashboardMetric { Title = "Total Orders", Value = ViewBag.TotalOrders.ToString(), Icon = "shopping-cart", Color = "dark" }
                };

            ViewBag.Metrics = metrics; // Pass metrics to the view
            //ViewBag.topProducts=_dashboardService.ProductsTopLastWeek();
            ViewBag.topProducts = await _dashboardService.ProductsTopLastWeekWithPercentage();
